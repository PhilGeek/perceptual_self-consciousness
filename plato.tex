...
\end{enumerate} 
Notice that reflexivity is argued against not by insisting that the object of an intentional psychic power is distinct from that power and its exercise. Insisting on the transcendent nature of intentional objects seems, at the very least, dangerously close to simply denying reflexivity. And Critias might reasonably think that he has grounds to revive his earlier charge of eristic refutation. The argument rather proceeds with the thought that powers only apply to things with the requisite nature or being coupled with the denial that the power or its exercise lacks the requisite nature or being. The abstract schema, while useful in making vivid the incoherence at the heart of the \emph{aporia}, can, however, be misleading. The crucial denial—that perception lacks the requisite nature or being—is never explicitly stated.
...
It is unclear what nature or being is required for the powers of motion and burning to apply (in contrast with the enumerated quantitative powers, 168c9–10). Perhaps burning applies only to the flammable, but as ``flammable'' simply means capable of being burned, this is not very informative. And perhaps the power of motion only applies to the movable, but, again, this is not very informative.

A motion that moves itself or heat that burns itself seems, at the very least, strange (\emph{atopon}). So it is understandable that these may appear incredible to some. But why does it no so appear to all? I have no idea what, precisely, Plato meant us to understand by this. Perhaps no determinate thing. So let us consider a more general, and tractable, question: Why might it not so appear to some? With respect to self-motion, we need look no further than the Platonic corpus. And with respect to self-burning, Theophrastus will later provide a rationale in \emph{De Igne}.
...
Though Socrates explicitly addresses quantitative, perceptual, and natural powers, he never explicitly addresses Critias' account of \emph{sōphrosunē}. Indeed, in a concessive move, the possibility of the self-knowledge with which Critias' identifies \emph{sōphrosunē} is left open, Socrates suggesting that we consider, instead, its benefit. Critias is left, if not puzzled, then stupefied. So Critias, at least, feels the force of The Argument from Relatives even if Socrates has not explicitly spelled out the implications for self-knowledge. The writing was on the wall. The reflexive quantitative powers are impossible, the reflexive psychic powers and the reflexive natural powers are at least open to serious doubt. But there remains no decisive verdict with respect ot the possibility of Critias' account of \emph{sōphrosunē}.

So Socrates leaves it open whether the self-knowledge with which he identifies \emph{sōphrosunē} is, in the end, possible, opting, instead, to consider the benefit of \emph{sōphrosunē} as Critias conceives of it.

A remarkable feature of the text suggests that this is not merely a dialectical concession by Socrates, but perhaps represents Plato's position at least at the time of the writing of the dialogue. 
